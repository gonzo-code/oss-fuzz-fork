...
(cd $OUT/corpus/tokeniser && find . -type f -name '*.html' | xargs zip -q -9 $OUT/TokeniserFuzzer_seed_corpus.zip)
(cd $OUT/corpus/tree && find . -type f -name '*.html' | xargs zip -q -9 $OUT/TreeBuilderFuzzer_seed_corpus.zip)
(cd $OUT/corpus/cleaner && find . -type f -name '*.html' | xargs zip -q -9 $OUT/CleanerFuzzer_seed_corpus.zip)
rm -rf $OUT/corpus
...
for fuzzer in $(find "$SRC" -maxdepth 1 -name '*Fuzzer.java'); do
  ...
  if [[ "$fuzzer_basename" == "TokeniserFuzzer" || \
        "$fuzzer_basename" == "TreeBuilderFuzzer" || \
        "$fuzzer_basename" == "CleanerFuzzer" ]]; then
